...
pub async fn initialize_flash(flash: &mut Flash) -> FlashToken {
    let address = &raw const RESERVED_FLASH as u32;
    defmt::debug!("Settings flash is at address 0x{:x}", &raw const RESERVED_FLASH);
...
pub async fn flash_task(flash: Flash, token: FlashToken) {
    ...
    loop {
        ...
        let timer_future = Timer::after(<crate::Used as Keyboard>::FLASH_APPLY_TIME);

        pin_mut!(receive_future);
        pin_mut!(timer_future);

        match select(receive_future, timer_future).await {
            Either::Left((operation, _)) => {
                match operation {
            ...
            Either::Right(..) => {
                if apply_flags.contains(ApplyFlags::ERASE) {
                    ...
                    let erase_end_address = token.address + core::mem::size_of::<ReservedFlash>() as u32;
                    defmt::unwrap!(flash.erase(token.address, erase_end_address).await);
                }
