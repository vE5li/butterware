...
use crate::hardware::ConfiguredPeripherals;
...
async fn main(spawner: Spawner) -> ! {
    ...
    let config = nrf_softdevice::Config {
        ...
        common_vs_uuid: Some(ble_common_cfg_vs_uuid_t { vs_uuid_count: 14 }),
        ..Default::default()
    };
    ...
    let ConfiguredPeripherals {
        mut matrix_pins,
        power_pin,
        #[cfg(feature = "lighting")]
        leds,
    } = keyboard.initialize_peripherals(peripherals).await.to_pins();
    ...
    defmt::unwrap!(spawner.spawn(power::power_task(power_pin)));
    ...
    loop {
        ...
        // connection again.
        #[cfg(feature = "left")]
        // FIX: This call makes the per-key leds not work for some reason
        ...
        keyboard.sides_disconnected().await;
