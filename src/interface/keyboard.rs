    ...
    [(); Self::COLUMNS * Self::ROWS * 2]:,
{
    ...
    /// Bluetooth address that the master will present itself with to other devices.
    ...
    /// data. Unless explicitly stated, this does not need to be increased.
    ...
    /// Function that gets called before initializing the peripherals. This is only run once on boot.
    ...
    /// Function that gets called after initializing the peripherals. This is only run once on boot.
