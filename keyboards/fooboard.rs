...
register_layers!(Fooboard, MeboardLayers, [BASE, SPECIAL]);

impl Fooboard {
    const SPE_SPC: Mapping = Mapping::layer_or_key(MeboardLayers::SPECIAL, SPACE);

    #[rustfmt::skip]
    const BASE: [Mapping; <Fooboard as Scannable>::COLUMNS * <Fooboard as Scannable>::ROWS] = [
        Q, W, F, P, B,
        A, R, S, T, G,
        Z, X, C, D, V,
    ...
    #[rustfmt::skip]
    const SPECIAL: [Mapping; <Fooboard as Scannable>::COLUMNS * <Fooboard as Scannable>::ROWS] = [
        N1, N2, N3, N4, N5,
        A, R, S, T, G,
        Z, X, C, D, V,
...
impl Scannable for Fooboard {
    const COLUMNS: usize = 5;
    const ROWS: usize = 4;
}

impl Keyboard for Fooboard {
    const DEVICE_NAME: &'static str = "Fooboard";
